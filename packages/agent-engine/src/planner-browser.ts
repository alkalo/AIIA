...
export class PlannerAgent {
  ...
  async plan(
    ...
    return {
      ...
      filters: {
        ...
        dedupe: parsed.filters?.dedupe ?? { enabled: true, fields: dedupeFields },
      },
      output: {
        schema,
  ...
  async suggestImprovements(
    ...
    return coerceJsonObject<Partial<AgentSpec>>(response) ?? {};
  }
}
...
export function diffSpecs(oldSpec: AgentSpec, newSpec: AgentSpec): Record<string, { old: unknown; new: unknown }> {
  ...
  for (const key of keys) {
    ...
    if (o !== n) {
      diff[key as string] = { old: oldSpec[key as keyof AgentSpec], new: newSpec[key as keyof AgentSpec] };
    }
  }
  return diff;
}
